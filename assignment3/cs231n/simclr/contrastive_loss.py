...
def sim(z_i, z_j):
    ...
    norm_dot_product = torch.dot(z_i, z_j) / ( torch.norm(z_i, p=2) *torch.norm(z_j, p=2) )
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for j in range(2*N) : 
          if j is not k : 
            denom1 += torch.exp(sim(z_k, out[j])/tau)
        l1 = -torch.log(num / denom1)

        denom2 = 0
        for j in range(2*N) : 
          if j is not k+N : 
            denom2 += torch.exp(sim(z_k_N, out[j])/tau)
        l2 = -torch.log(num / denom2)
...
def sim_positive_pairs(out_left, out_right):
    ...
    out_left_n = out_left/torch.linalg.norm(out_left, dim=1, keepdim=True)
    out_right_n = out_right/torch.linalg.norm(out_right, dim=1, keepdim=True)
    pos_pairs = torch.mul(out_left_n, out_right_n).sum(dim=1, keepdim=True)
...
def compute_sim_matrix(out):
    ...
    out_n = out/torch.linalg.norm(out, dim=1, keepdim=True)
    sim_matrix = torch.matmul(out_n, out_n.T)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    loss = torch.sum(-torch.log(numerator / denom[:N]) -torch.log(numerator / denom[N:]))
    loss /= 2*N
